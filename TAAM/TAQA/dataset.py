...
class VideoDataset(Dataset):
    ...
    def __init__(self, mode, args):
        ...
        # loading annotations
        self.args = args
        self.annotations = pkl.load(open(os.path.join(info_dir, 'annotations.pkl'), 'rb'))
        self.keys = pkl.load(open(os.path.join(info_dir, f'{self.mode}_split.pkl'), 'rb'))

    def proc_label(self, data):
        # Each judge choose a score from [0, 0.5, ..., 9.5, 10], we normalize it into 0~20
        tmp = [stats.norm.pdf(np.arange(output_dim['TAAM']), loc=judge_score * (output_dim['TAAM']-1) / judge_max, scale=self.args.std).astype(np.float32)
    ...
    def get_imgs(self, key):
        ...
        images = torch.zeros(num_frames, C, H, W)    #103 c h w

        lenth_of_images = len(image_list)
        for j, i in enumerate(sample_range):
            if self.mode == 'train':
                images[j] = load_image_train(image_list[i%lenth_of_images], hori_flip, transform)

            if self.mode == 'test':
                images[j] = load_image(image_list[i%lenth_of_images], transform)
        return images
